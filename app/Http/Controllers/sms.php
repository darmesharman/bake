...
{
    $url = $_url = (getenv('SMSC_HTTPS') ? "https" : "http")."://smsc.kz/sys/$cmd.php?login=".urlencode(getenv('SMSC_LOGIN'))."&psw=".urlencode(getenv('SMSC_PASSWORD'))."&fmt=1&charset=".getenv('SMSC_CHARSET')."&".$arg;
...
{
    $ret = "";
    $post = getenv('SMSC_POST') || strlen($url) > 2000 || $files;
    ...
    elseif ($files) {
        ...
        if (!getenv('SMSC_HTTPS') && function_exists("fsockopen"))
        {
            $m = parse_url($url);
